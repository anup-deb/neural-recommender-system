...
class Autorec(nn.Module):
    def __init__(self,args, num_users,num_items):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.num_users, self.hidden_units),
            nn.Sigmoid()
        )

        self.decoder = nn.Sequential(
            nn.Linear(self.hidden_units, self.num_users),
        )
...
def test(epoch, dataset):
    ...
    npdecoder = decoder.cpu().detach().numpy()
    nptest_mask = test_mask_r_tensor.cpu().detach().numpy()

    num_users = npdecoder.shape[1]
    NDCG = []
    HR = []
    for i in range(num_users):
        user_preds = npdecoder[:,i]
        user_mask = nptest_mask[:,i]
        user_unrated = np.where(user_mask == 0)[0]

        random100 = np.random.choice(user_unrated, 100)

        user_rated = np.where(user_mask != 0)[0]

        if (user_rated.size == 0):
            continue
        random1 = np.random.choice(user_rated, 1)

        user_ns_predictions = user_preds[random100]
        user_item_prediction = user_preds[random1]

        listed_preds = user_ns_predictions.tolist()
        listed_preds.append(user_item_prediction)
        unranked_preds = np.array(listed_preds)
        sorted_inds = np.argsort(unranked_preds)[:10]
    ...
    print('epoch ', epoch, ' test RMSE : ', RMSE, ' HR : ', np.mean(HR), ' NDCG', np.mean(NDCG))
    return RMSE
...
if __name__ == '__main__':
    ...
    train_r,train_mask_r,test_r, test_mask_r = train_r.T,train_mask_r.T,test_r.T,test_mask_r.T
    ...
    torch.save(rec, f"runs/autorec_{args.outputname}.pth")
